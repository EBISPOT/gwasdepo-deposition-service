...
public class SummaryStatsValidationServiceImpl implements SummaryStatsValidationService {
    ...
    @Async
    @Override
    public void validateSummaryStatsData(Submission submission) {
        ...
        if (templateFile == null || dataFile == null) {
            ...
            markInvalidFile(dataFile, submission, ErrorType.NO_TEMPLATE, null, null, null);
            return;
        }
        ...
        if (dataReader.isValid()) {
            ...
            if (dataReader.getSchemaVersion() == null) {
                this.markInvalidFile(dataFile, submission, ErrorType.NO_SCHEMA_PRESENT, null, dataReader, schemaReader);
                return;
            }
            ...
            if (!schemaReader.getSchemaVersion().equalsIgnoreCase(dataReader.getSchemaVersion()) ||
                ...
                markInvalidFile(dataFile, submission, ErrorType.TEMPLATE_MISMATCH, null, dataReader, schemaReader);
                return;
            }
            ...
            if (templateService != null) {
                ...
                if (templateSchemaResponseDto == null) {
                    ...
                    markInvalidFile(dataFile, submission, ErrorType.NO_SCHEMA_VERSION, dataReader.getSchemaVersion(), dataReader, schemaReader);
                    return;
                }
                if (templateSchemaResponseDto.getSubmissionTypes().containsKey(SubmissionType.SUMMARY_STATS.name())) {
                    ...
                    if (schema == null) {
                        ...
                        markInvalidFile(dataFile, submission, ErrorType.UNUSABLE_SCHEMA, null, dataReader, schemaReader);
                        return;
                    }
                } else {
                    log.error("Template service does not have a schema for the provided version: {}", dataReader.getSchemaVersion());
                ...
                markInvalidFile(dataFile, submission, ErrorType.NO_TEMPLATE_SERVICE, null, dataReader, schemaReader);
                return;
            }
        ...
        markInvalidFile(dataFile, submission, ErrorType.INVALID_FILE, dataFile.getFileName(), dataReader, schemaReader);
    }
